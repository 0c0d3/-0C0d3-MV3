...
(function() {
  ...
  const scriptlets = {
    ...
    'remove-reddit-login': function() {
      function clean() {
        const selectors = ['div[role="dialog"]','._2SdH','._1hI6','[data-testid="login-prompt"]','.login-prompt','.login-overlay'];
  ...
  browser.runtime.onMessage.addListener(msg => {
    if (msg.type === 'executeScriptlet' && scriptlets[msg.name]) {
      try { scriptlets[msg.name](...(msg.args || [])); } catch(e) {}
    }
  });

  const defaults = ['remove-youtube-ads', 'block-ad-fetch', 'remove-reddit-login'];
  for (const name of defaults) {
    if (scriptlets[name]) try { scriptlets[name](); } catch(e) {}
  }
})();